...
import { getTimeIn12HourFormat } from "@/app/(withDashboardLayout)/dashboard/doctor/components/DoctorSchedulesModal";
import { useCreateAppointmentMutation } from "@/redux/api/appoinment.api";
import { useGetAllDoctorSchedulesQuery } from "@/redux/api/doctorScheduleApi.api";
import { useInitialPaymentMutation } from "@/redux/api/payment.api";
  ...
  const handleBookAppointment = async () => {
    try {
      if (id && scheduleId) {
        ...
        console.log(res, "create appointment res");

        if (res.id) {
          const response = await initialPayment(res.id).unwrap();
          console.log(response, "pyment response ");
